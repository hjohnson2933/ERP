...
export default async function BrandsPage() {
  ...
  const { data: brands, error } = await erp
    .from("brands")
    .select("id, brand_code, name, notes, active")
    .is("deleted_at", null)
    .order("name", { ascending: true })
    .returns<Pick<Brand, "id" | "brand_code" | "name" | "notes" | "active">[]>();
          ...
          <tbody>
            {(brands ?? []).map((b) => (
              <tr key={b.id} className="border-b border-ink-border last:border-0">
                ...
                <td className="px-3 py-2 font-mono text-xs text-ink-muted">{b.brand_code || "—"}</td>
            ...
            {(brands ?? []).length === 0 && (
              <tr>
                <td colSpan={canManage ? 5 : 4} className="px-3 py-6 text-center text-ink-muted">
